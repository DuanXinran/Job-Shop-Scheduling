...
export default class Canvas1 extends Component{
	...
	shouldComponentUpdate(nextProps, nextState) {
		...
		if (this.state.canvasData !== nextState.canvasData) {
	  this.mount();
		  return true;
	  ...
	  mount(){
		...
		var ob = JSON.parse(this.state.canvasData.data);
        var array=ob["returnArray"];
        var x=ob["x"];//获取x
        var y=ob["y"];//获取y
        for(let i=0;i<y;i++)
        {
		...
		/*输入信息区*/
		  ...
		  var mach =3;//颜色最大数量：作业数：jobnumber
		  var xstr="作业";//y轴信息
		  var smallstr="设备";//小标信息
		  var ret =  [[1, 1, 1, 2, 2, -1, -1, -1, -1, -1, -1,], //数据信息：returnArray
    ...
    render(){
        ...
        return(
            <div className={this.props.isShow1 ? 'show': 'hidden'}  onChange={this.change}>
